...
api.getUsageStats = async (days) => {
    ...
    for (const r of rows) {
        ...
        if (!byModel[mid])
            byModel[mid] = { modelId: mid, modelName: r.model_name, calls: 0, promptTokens: 0, completionTokens: 0, totalTokens: 0, cost: 0, latencyMs: 0 };
        byModel[mid].calls++;
        ...
        if (!byDay[day])
            byDay[day] = { date: day, calls: 0, totalTokens: 0, cost: 0 };
        byDay[day].calls++;
