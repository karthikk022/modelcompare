    ...
    app.get('/api/benchmarks/sync', handle(async (req, res) => {
        ...
        for (const m of models) {
                ...
                if (!/openai|anthropic|google|xai|x-ai|deepmind/.test(slug)) {
                    ...
                    for (const c of candidates) {
                        try {
                            ...
                            console.warn('[benchmark] HF model lookup failed:', e.message);
                        }
                    }
                }
            }
            ...
            try {
                ...
                console.warn('[benchmark] HF results fetch failed:', e.message);
            }
        }
        if (updated > 0)
            await db.snapshotAllModels('benchmark-sync');
