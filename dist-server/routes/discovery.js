...
function cleanModelName(raw, author) {
    ...
    if (name.startsWith(author + '/'))
        name = name.slice(author.length + 1);
    for (const ptn of _CLEAN_SUFFIXES)
        name = name.replace(ptn, '');
    name = name.replace(/[-_]/g, ' ').trim();
    if (name.length < 3 || /^[a-f0-9]{7,}$/i.test(name))
        return null;
...
function inferTags(name, desc, benchmarks, arch) {
    ...
    if (/gpt-4|gpt4|claude-3\.5|gemini-2|llama-4|command-a|deepseek.*v[34]|qwen.*?3/.test(nl))
        tags.add('flagship');
    if (/mini|small|lite|compact|nano|pico|tiny|lite|flash|haiku/.test(nl))
        tags.add('budget');
    if (/llama|mistral|qwen|deepseek|phi|olmo|falcon|gemma|bloom|mpt|dbrx|solar|yi-/.test(nl) && !/claude|gpt|gemini|command/.test(nl))
        tags.add('open-source');
    if (/coder|code|swe|dev|infi|codestral/.test(nl))
        tags.add('coding');
    if (/reason|think|deep.*think|r1|o1|o3/.test(nl) || hasBench('gpqa') || arch === 'DeepseekR1')
        tags.add('reasoning');
    if (/math|math-?500|gsm8k/.test(nl) || hasBench('math-500') || hasBench('gsm8k'))
        tags.add('math');
    if (/nemo|mamba|gemma-2|phi-3|phi-4|stable/.test(nl))
        tags.add('speed');
    if (/agent|tool|function|computer-use/.test(nl))
        tags.add('agentic');
    if (/multilingual|chinese|japanese|korean|arabic|euro|translation/.test(dl) || /qwen|yi-|bge|jais|instruct/.test(nl))
        tags.add('multilingual');
    if (/context|long|128k|1m|1mio|infinite|document/.test(dl) || /128k|1m|200k/.test(nl))
        tags.add('long-context');
    if (/vision|multi.?modal|image|video|audio|speech|whisper|clip|siglip|blip/.test(nl) || /image|vision|audio|video/.test(dl))
        tags.add('multimodal');
    if (/expert|knowledge|world|sci|bio|med|law|finance|legal/.test(dl) || hasBench('mmlu'))
        tags.add('knowledge');
    if (!tags.size)
        tags.add('other');
    return [...tags];
...
async function discoverFromHF(limit) {
    ...
    for (const item of list) {
        ...
        if (!cleaned || _SKIP_PATTERNS.test(item.id))
            continue;
        if ((item.likes || 0) < 10)
            continue;
    ...
    async function processItem(item) {
        ...
        try {
            ...
            console.warn('[discover] HF results fetch failed:', e.message);
        }
        ...
        try {
            ...
            if (config.config) {
                ...
                if (c.model_type)
                    arch = c.model_type;
                if (c.num_parameters)
                    params = (c.num_parameters / 1e9).toFixed(1) + 'B';
                    ...
                    if (p > 0.1)
                        params = Math.round(p) + 'B (est.)';
                }
            }
        }
        catch (e) {
            console.warn('[discover] HF config fetch failed:', e.message);
        }
...
async function discoverFromOpenRouter(limit) {
    ...
    for (const item of items) {
        try {
            ...
            console.warn('[discover] OpenRouter item failed:', e.message);
        }
    }
    return results;
}
function register(app) {
    app.get('/api/discover', handle(async (req, res) => {
        ...
        try {
            ...
            const seen = new Set();
            models = models.filter(m => { const k = m.name.toLowerCase(); if (seen.has(k))
    ...
    app.get('/api/live-pricing', handle(async (req, res) => {
        ...
        try {
            ...
            if (data.fetchedAt)
                await db.snapshotAllModels('pricing-refresh');
            res.json(data);
        }
        catch (e) {
