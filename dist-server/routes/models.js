...
function checkFields(isUpdate) {
    return body().custom(body => {
        if (!isUpdate && (!body.id || typeof body.id !== 'string'))
            throw new Error('Model must have a string id');
        if (!isUpdate && (!body.name || typeof body.name !== 'string'))
            throw new Error('Model must have a string name');
        for (const key of Object.keys(body)) {
            if (!VALID_MODEL_KEYS.has(key))
                throw new Error('Unknown field: ' + key);
            if (NUMERIC_FIELDS.has(key) && body[key] != null && typeof body[key] !== 'number')
                throw new Error(key + ' must be a number');
            if (ARRAY_FIELDS.has(key) && body[key] != null && !Array.isArray(body[key]))
                throw new Error(key + ' must be an array');
            if (OBJECT_FIELDS.has(key) && body[key] != null && (typeof body[key] !== 'object' || Array.isArray(body[key])))
...
function register(app) {
    ...
    app.get('/api/models', handle(async (req, res) => {
        ...
        if (q)
            models = models.filter(m => m.name.toLowerCase().includes(q) || m.id.toLowerCase().includes(q) || (m.description || '').toLowerCase().includes(q) || (m.tags || []).some(t => t.toLowerCase().includes(q)));
        if (provider)
            models = models.filter(m => (m.provider || '').toLowerCase().includes(provider));
        if (tag)
            models = models.filter(m => (m.tags || []).some(t => t.toLowerCase() === tag));
    ...
    app.get('/api/models/export', handle(async (req, res) => {
        ...
        if (format === 'csv') {
            ...
            const esc = v => { if (v == null)
                return ''; const s = String(v).replace(/^[=+\-@]/, "'$&"); return s.includes(',') || s.includes('"') || s.includes('\n') ? '"' + s.replace(/"/g, '""') + '"' : s; };
            const rows = models.map(m => cols.map(c => {
                if (c === 'tags')
                    return esc((m.tags || []).join('; '));
                if (c.startsWith('bench_')) {
                    const k = c.slice(6).replace(/_/g, ' ');
                    return esc(m.benchmarks ? m.benchmarks[k] : '');
                }
                if (c.startsWith('score_')) {
                    const k = c.slice(6);
                    return esc(m.scores ? m.scores[k] : '');
                }
                return esc(m[c]);
            }).join(','));
            const csv = '\uFEFF' + cols.join(',') + '\n' + rows.join('\n');
    ...
    app.post('/api/models', bodyIsObject(), checkFields(false), requireValid, handle(async (req, res) => {
        if (await db.getModel(req.body.id))
    ...
    app.patch('/api/models/:id/slug', param('id').isString().notEmpty().withMessage('Model id is required'), body('slug').isString().notEmpty().withMessage('Slug is required'), requireValid, handle(async (req, res) => {
        const model = await db.getModel(req.params.id);
        if (!model)
            return res.status(404).json({ error: 'Model not found' });
    ...
    app.put('/api/models/:id', param('id').isString().notEmpty().withMessage('Model id is required'), bodyIsObject(), checkFields(true), requireValid, handle(async (req, res) => {
    ...
    app.get('/api/recommend', handle(async (req, res) => {
        ...
        const scored = models.map(m => {
            ...
            for (const word of taskWords) {
                if (m.bestFor && m.bestFor.some(t => t.includes(word)))
                    score += 3;
                if (m.strengths && m.strengths.toLowerCase().includes(word))
                    score += 2;
                if (m.name.toLowerCase().includes(word))
                    score += 1;
            }
            if (m.scores) {
                for (const [cat, val] of Object.entries(m.scores)) {
                    const catWords = cat.replace(/([A-Z])/g, ' $1').toLowerCase();
                    if (taskWords.some(w => catWords.includes(w)))
                        score += val / 100;
                }
            }
