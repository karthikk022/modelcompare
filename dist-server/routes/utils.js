...
function findModelMatch(name, models) {
    ...
    for (const m of models) {
        ...
        if (ml.includes(nl) || nl.includes(ml))
            return m;
    }
    ...
    if (words.length > 1) {
        for (const m of models) {
            ...
            if (hits >= Math.ceil(words.length * 0.5))
                return m;
        }
    }
    return null;
...
async function fetchLivePricing(force) {
    if (!force && _livePricingCache && _livePricingTime && Date.now() - _livePricingTime < _LIVE_TTL)
        return _livePricingCache;
    try {
        ...
        if (_livePricingCache)
            return { ..._livePricingCache, error: e.message };
